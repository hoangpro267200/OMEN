...
class CursorPagination(BaseModel, Generic[T]):
    ...
    prev_cursor: Optional[str] = Field(
        None, description="Cursor for previous page (null if first page)"
    )
    has_more: bool = Field(..., description="Whether there are more items")
    total_count: Optional[int] = Field(
        None, description="Total count (optional, may be expensive to compute)"
    )
...
def create_page_response(
    ...
    if has_more and items:
        ...
        cursor_data = CursorData(
            ...
            last_timestamp=(
                get_item_timestamp(last_item).isoformat() if get_item_timestamp else None
            ),
            direction="forward",
        )
    ...
    if prev_cursor_data and items:
        ...
        cursor_data = CursorData(
            ...
            last_timestamp=(
                get_item_timestamp(first_item).isoformat() if get_item_timestamp else None
            ),
            direction="backward",
        )
...
class PaginationParams(BaseModel):
    ...
    cursor: Optional[str] = Field(None, description="Cursor from previous response for pagination")
    limit: int = Field(50, ge=1, le=100, description="Number of items per page (max 100)")
