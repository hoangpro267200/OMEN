...
class PostgresSignalRepository(SignalRepository):
    ...
    async def initialize(self) -> None:
        ...
        logger.info("PostgreSQL repository initialized with pool size %d-%d", 
    ...
    def find_by_id(self, signal_id: str) -> Optional[OmenSignal]:
        ...
        return asyncio.get_event_loop().run_until_complete(
            self.find_by_id_async(signal_id)
        )
    ...
    def find_by_event_id(self, event_id: str) -> list[OmenSignal]:
        ...
        import asyncio
        return asyncio.get_event_loop().run_until_complete(
            self._find_by_event_id_async(event_id)
        )
    ...
    async def find_by_id_async(self, signal_id: str) -> Optional[OmenSignal]:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT payload FROM omen_signals WHERE signal_id = $1",
                signal_id
            )
            if row:
    ...
    async def find_by_hash_async(self, input_event_hash: str) -> Optional[OmenSignal]:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT payload FROM omen_signals WHERE input_event_hash = $1",
                input_event_hash
            )
            if row:
                return OmenSignal.model_validate_json(row["payload"])
        ...
        async with self._pool.acquire() as conn:
            if since:
                result = await conn.fetchval(
                    "SELECT COUNT(*) FROM omen_signals WHERE generated_at >= $1",
                    since
                )
            else:
