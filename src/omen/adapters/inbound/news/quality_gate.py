...
NEGATIVE_SENTIMENT_WORDS = frozenset([
    ...
    "disruption", "disrupted", "disrupting",
    "strike", "strikes", "striking",
    "blockade", "blocked", "blocking",
    "crisis", "crises",
    "threat", "threatens", "threatening",
    "conflict", "war", "warfare",
    "sanctions", "sanctioned",
    "embargo", "embargoed",
    "shortage", "shortages",
    "delay", "delays", "delayed",
    "closure", "closed", "closing",
...
class NewsQualityGate:
    ...
    def evaluate(
        ...
        if credibility < self._config.min_credibility:
            passed_gate = False
            rejection_reason = f"Credibility too low: {credibility:.2f} < {self._config.min_credibility}"
        elif recency < self._config.min_recency:
            passed_gate = False
            rejection_reason = f"Article too old: recency={recency:.2f} < {self._config.min_recency}"
        elif combined < self._config.min_combined_score:
            passed_gate = False
            rejection_reason = f"Combined score too low: {combined:.2f} < {self._config.min_combined_score}"
        elif is_duplicate:
            passed_gate = False
