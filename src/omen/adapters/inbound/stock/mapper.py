...
def _map_region_to_locations(region: str) -> list[GeoLocation]:
    ...
    region_map = {
        "us": [GeoLocation(name="United States", region="North America", latitude=37.0902, longitude=-95.7129)],
        "vietnam": [GeoLocation(name="Vietnam", region="Southeast Asia", latitude=14.0583, longitude=108.2772)],
        "global": [GeoLocation(name="Global", region="Global", latitude=0.0, longitude=0.0)],
        "europe": [GeoLocation(name="Europe", region="Europe", latitude=54.5260, longitude=15.2551)],
        "asia": [GeoLocation(name="Asia", region="Asia", latitude=34.0479, longitude=100.6197)],
    }
    return region_map.get(region, [GeoLocation(name="Global", region="Global", latitude=0.0, longitude=0.0)])
...
class StockMapper:
    ...
    def map_quote(
        ...
        market = MarketMetadata(
            ...
            current_liquidity_usd=float(quote.volume) * quote.price / 1000 if quote.price > 0 else 0.0,
            created_at=now,
        )
