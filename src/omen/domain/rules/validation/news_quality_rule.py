...
class NewsQualityGateRule(Rule[RawSignalEvent, ValidationResult]):
    ...
    def apply(self, signal: RawSignalEvent) -> ValidationResult:
        ...
        quality_score = combined if combined is not None else (
            (credibility or 0.5) * 0.6 + (recency or 0.5) * 0.4
        )
        
        # Bound the contribution
        bounded_score = min(quality_score, 1.0 - self._max_confidence_boost) + self._max_confidence_boost
    ...
    def explain(
        ...
        return ExplanationStep(
            ...
            confidence_contribution=output_data.score if output_data.status == ValidationStatus.PASSED else 0.0,
