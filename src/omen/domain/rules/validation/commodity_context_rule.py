...
class CommodityContextRule(Rule[RawSignalEvent, ValidationResult]):
    ...
    def apply(self, signal: RawSignalEvent) -> ValidationResult:
        ...
        severity_index = SEVERITY_ORDER.index(severity_value) if severity_value in SEVERITY_ORDER else 0
        min_severity_index = SEVERITY_ORDER.index(self._min_severity) if self._min_severity in SEVERITY_ORDER else 1
