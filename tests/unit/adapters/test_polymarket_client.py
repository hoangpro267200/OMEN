...
class TestFetchMarkets:
    ...
    def test_returns_list_on_success(
        self, client: PolymarketClient, mock_httpx_success
    ) -> None:
    ...
    def test_raises_rate_limited_on_429(
        self, client: PolymarketClient, mock_httpx_429
    ) -> None:
        with pytest.raises(SourceRateLimitedError) as exc_info:
            client.fetch_markets(limit=10)
        assert exc_info.value.retry_after_seconds == 60

    def test_raises_unavailable_on_500(
        self, client: PolymarketClient, mock_httpx_500
    ) -> None:
        with pytest.raises(SourceUnavailableError):
            client.fetch_markets(limit=10)

    def test_raises_unavailable_on_503(
        self, client: PolymarketClient, mock_httpx_503
    ) -> None:
        with pytest.raises(SourceUnavailableError):
            client.fetch_markets(limit=10)
...
class TestCircuitBreaker:
    ...
    def test_records_success_on_good_response(
        self, mock_httpx_success
    ) -> None:
        with patch(
            "omen.adapters.inbound.polymarket.client.create_source_circuit_breaker"
        ) as p_cb:
            mock_cb = MagicMock()
